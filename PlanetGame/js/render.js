...
function render(scene) {
    ...
    // This process is a bit odd, because there's a disconnect between pixels and
    // vectors: given the left and right, top and bottom rays, the rays we shoot
    // are just interpolated between them in little increments.
    ...
    // 90° right and one that's turned 90° up. Since the cross product
    ...
    var vpRight = Vector.unitVector(Vector.crossProduct(eyeVector, camera.up));
    ...
    for (var x = 0; x < width; x++) {
        for (var y = 0; y < height; y++) {
            ...
            var sampleRate = 1;
            color = Vector.ZERO;
            for (var s = -.4; s < .6; s += sampleRate) {
                for (var r = -.4; r < .6; r += sampleRate) {
                    ...
                    color = Vector.add(color, trace(ray, scene));
                }
            }
    ...
    for(x=0;x<width;x++){
        for(y=0;y <height;y++){
            index = (x * 3) + (y* width  * 3);
            var d_index = (x * 4) + ((height-1 -y)* width * 4);
