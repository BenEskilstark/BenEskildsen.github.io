...
function drawThePrimitive(gl, primitive) {
	...
	var viewMat;
    // position of eye, direction eye is looking, vector that points up
	if (primitive.name == "cube") {
		...
		viewMat = SglMat4.lookAt([0,-2, 5], [0,0,0], [0,1,1]);
    } else if (primitive.name == "sphere") {
        var x = parseInt(document.getElementById("cameraX").value);
        var y = parseInt(document.getElementById("cameraY").value);
        var z = parseInt(document.getElementById("cameraZ").value);

		viewMat = SglMat4.lookAt([x,y,z], [0,0,0], [0,1,1]);
    }
